...
const renderText = (text, className, baseWeight = 400) => {
    return [...text].map((char, i) => (
        <span
            key={i}
            className={className}
            style={{ fontWeight: baseWeight }} // <-- set initial font weight
        >
      {char === ' ' ? '\u00A0' : char}
    </span>
    ));
...
const setupTextHover = (container, type) => {
    ...
    const animateLetter = (letter, weight, duration = 0.25) => {
        gsap.to(letter, {
            duration,
            ease: 'power2.out',
            fontWeight: weight // <-- animate fontWeight
        });
    };
    ...
    const handleMouseLeave = () => {
        letters.forEach((letter) => animateLetter(letter, FONT_WEIGHTS[type].default, 0.3));
    }

    container.addEventListener('mousemove', handleMouseMove);
    container.addEventListener('mouseleave', handleMouseLeave);

    // cleanup for StrictMode double-mount
    return () => {
